# parseo_atom.py - Version 7 DEFINITIVA
# Correcciones: ciudades sin parentesis, expedientes forzados como texto
...
MAP_PROV_CCAA = {
    '01': 'Pais Vasco',         '02': 'Castilla-La Mancha',
    '03': 'Comunidad Valenciana','04': 'Andalucia',
    '05': 'Castilla y Leon',    '06': 'Extremadura',
    '07': 'Baleares',           '08': 'Cataluna',
    '09': 'Castilla y Leon',    '10': 'Extremadura',
    '11': 'Andalucia',          '12': 'Comunidad Valenciana',
    '13': 'Castilla-La Mancha', '14': 'Andalucia',
    '15': 'Galicia',            '16': 'Castilla-La Mancha',
    '17': 'Cataluna',           '18': 'Andalucia',
    '19': 'Castilla-La Mancha', '20': 'Pais Vasco',
    '21': 'Andalucia',          '22': 'Aragon',
    '23': 'Andalucia',          '24': 'Castilla y Leon',
    '25': 'Cataluna',           '26': 'La Rioja',
    '27': 'Galicia',            '28': 'Madrid',
    '29': 'Andalucia',          '30': 'Murcia',
    '31': 'Navarra',            '32': 'Galicia',
    '33': 'Asturias',           '34': 'Castilla y Leon',
    '35': 'Canarias',           '36': 'Galicia',
    '37': 'Castilla y Leon',    '38': 'Canarias',
    '39': 'Cantabria',          '40': 'Castilla y Leon',
    '41': 'Andalucia',          '42': 'Castilla y Leon',
    '43': 'Cataluna',           '44': 'Aragon',
    '45': 'Castilla-La Mancha', '46': 'Comunidad Valenciana',
    '47': 'Castilla y Leon',    '48': 'Pais Vasco',
    '49': 'Castilla y Leon',    '50': 'Aragon',
    '51': 'Ceuta',              '52': 'Melilla',
}
...
# ── TABLA GEO INE ────────────────────────────────────────────────────────────
RUTA_GEO = r"C:\proyectos\licitaciones\datos\Sector Publico\geo\municipios_ine.csv"

def descargar_geo_ine():
    os.makedirs(os.path.dirname(RUTA_GEO), exist_ok=True)
    if os.path.exists(RUTA_GEO):
        return
    print("  Descargando tabla de municipios del INE...")
    ...
    try:
        ...
        with open(RUTA_GEO, 'wb') as f:
            f.write(r.content)
        print("  Tabla INE descargada correctamente.")
    except Exception as e:
        print(f"  No se pudo descargar la tabla INE: {e}. Se usara fallback por NUTS.")
...
def cargar_geo_ine():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            provincia = MAP_PROV.get(cod_prov, 'Sin dato')
            ccaa      = MAP_PROV_CCAA.get(cod_prov, 'Sin dato')
            lookup[nombre_norm] = (nombre, provincia, ccaa)
...
def limpiar_ciudad(texto):
    """Elimina contenido entre parentesis y espacios extra."""
    if not texto or texto == 'Sin dato':
        return 'Sin dato'
    sin_parentesis = re.sub(r'\s*\(.*?\)', '', texto)
    resultado = ' '.join(sin_parentesis.split()).strip()
...
def resolver_geo(nuts_raw, ccaa_raw, ciudad_raw):
    """Resuelve ciudad, provincia y CCAA. Prioridad: INE > NUTS > XML."""
    lookup     = get_geo_lookup()
    ciudad_limpia = limpiar_ciudad(ciudad_raw).title()

    # 1. Tabla INE por nombre de municipio
    ...
    if nuts_raw and nuts_raw != 'Sin dato':
        nuts_limpio = nuts_raw.strip().upper()[:5]
        ccaa = MAP_NUTS_CCAA.get(nuts_limpio, 'Sin dato')
        if ccaa != 'Sin dato':
            return ciudad_limpia, 'Sin dato', ccaa

    # 3. Campo CCAA del XML
...
if __name__ == "__main__":
    entrada = r"C:\proyectos\licitaciones\datos\Sector Publico\raw\2026\03\licitacionesPerfilesContratanteCompleto3.atom"
    salida  = r"C:\proyectos\licitaciones\datos\Sector Publico\csv\2026\03\licitaciones_final.csv"

    df = parsear_atom(entrada)

    print("\n── Primeras 5 licitaciones ──")
    cols = ['expediente', 'estado', 'ciudad', 'provincia', 'ccaa', 'importe_efectivo']
    print(df[cols].head(5).to_string())

    print("\n── CCAA mas frecuentes ──")
    print(df['ccaa'].value_counts().head(10).to_string())

    print("\n── Provincias mas frecuentes ──")
    print(df['provincia'].value_counts().head(10).to_string())

    print("\n── Licitaciones con importe efectivo = 0 ──")
    sin_imp = df[df['importe_efectivo'] == 0][['expediente', 'importe_total', 'importe_estimado']].head(5)
    print(sin_imp.to_string())
